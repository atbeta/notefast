...
- **CJK 问句后缀剥离 + 全半角归一（查询端）**：`lexicalSearch` 的 `cjkCoreTerm` 除前缀剥离（「什么是XXX」）外新增**尾部问句剥离**（`CJK_QUERY_SUFFIX_RE`：「向量数据库怎么选」「红烧肉怎么做才好吃」「卡片笔记应该怎么写」→ 核心词，容忍「应该/要/想」中缀与「才/要/更」+ 短尾补语；中置式「怎么选向量数据库」不处理，留给语义路/词典）。**全半角**：core `fullToHalfWidth`（全角 ASCII 区 → 半角）用于查询 term 归一（`lexicalSearch` terms、`entitySearch` 候选名）；LIKE 组内每个 variant 再补 `halfToFullPunct` 标点/数字全角形态（**字母保持半角**——中文文档惯例是「RAG（检索增强生成）」混合写法），Cond/Params 共用 `groupForms` 展开保证占位符一一对应；词典匹配键 `dictKey = fullToHalfWidth(normalizeEntityName(x))`（**实体表归并键 normalizeEntityName 不含全半角转换**，存量稳定）。eval fixtures 已补 3 篇语料（RAG/检索增强、sqlite-vec 使用、向量数据库选型）+ 10 条查询（suffix_question/abbr_expansion/width_variant/alias_synonym 类型），mock 全量 70 条可跑；「向量库怎么用」类同义词查询与「sqlite_vec」连接符变体在无词典时词法 miss 属预期（term-dict/语义路兜底，评测注释已写明）
