...
- Markdown 富渲染：Mermaid 与 LaTeX 均已接入（LaTeX 见下方实现要点）；编辑器侧已补块级公式预览 widget（cm/mathPreview.ts）与桌面端选区气泡（问 AI / 改写流式原地替换，components/editor/SelectionBubble.tsx）
...
- **窗口 chrome 方向**：Tauri TitleBar 已删应用名文字（左侧只留拖拽区+窗口控制，消除与侧栏品牌行重复）；**终态=向 macOS 收敛的一条带**（窗口控制浮在「侧栏品牌行+内容顶栏」拼成的带上，砍 h-9 整条 TitleBar，注意 AIChatPanel top-9 联动与内容顶栏右侧 ~130px 净空），待 chrome 打磨窗口期实施；参考形态=RelayCraft 单顶带（但其居中命令栏/纯图标栏不适合 NoteFast）
...
- **外部采集通道（capture，连接器对接基础）**：统一走 `POST /import/markdown`（契约只加不改）：`notebook_id` 已可选（缺省 `resolveNotebookId` 取第一个笔记本）、tags 入库前 normalize（`services/docImport.ts` 的 `normalizeDocTags`，docs.ts 同用）；source provider 命名 `web-clipper`（bookmarklet）/ `ios-shortcut`（快捷指令），`file-open` 已占用勿动，external_id 网页类用规范化 URL（去 hash/utm_*，`web/src/lib/bookmarklet.ts` 的 `normalizeClipUrl` 与生成代码内嵌同规则实现）。**CORS**：`CORS_ORIGINS` 任一项为字面 `*` 时给 hono cors 传字符串 `'*'`（数组语义下 `'*'` 不匹配任何 origin，这是修复点），`allowMethods` 已补 PUT；**免鉴权 + `*` = 任意网页可读写全库**（README/docs 均有警示，仅限本地/内网；该组合启动时 index.ts 另有醒目告警）。**api_tokens scopes 已强制**：`middleware/auth.ts` 方法级拦截——非 GET/HEAD/OPTIONS 且 token scopes 不含 write → 403（env API_TOKEN / api.key / web-session / trustedLocal 直通不受影响；**MCP `POST /mcp` 全 POST，只读 token 同样被挡**）。Web 设置页 tokens tab 底部 `CapturePanel`（bookmarklet 生成器 + iOS 快捷指令/curl 示例；生成逻辑在 `lib/bookmarklet.ts` 纯函数有单测；token 明文嵌书签代码，UI 引导用可撤销 api_token）；使用文档 `docs/capture.md`。**明确不做服务端 URL 抓取**（readability+DOM+SSRF 防护成本，客户端带正文已覆盖且能拿登录态内容；URL 已在 source.external_id 留位）
