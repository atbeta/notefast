...
- Markdown 富渲染：Mermaid 与 LaTeX 均已接入（LaTeX 见下方实现要点）；编辑器选区气泡后延
...
- **实体重复处理：信号分桶，自动合拼写变体 + 词典建议，不逐对人工合并**（`store/entities.ts` 的 `findPotentialDuplicates` + `api/entities.ts`）：`DuplicateGroup.signal` 分两类——**typo**（纯 ASCII 名、长度 ≥5、编辑距离 ≤2，qdrant/qdrnt）高置信同义 → 实体页加载时 `POST /api/v1/entities/duplicates/auto-merge` 自动合（有副作用故 POST 不挂 GET；合并方向 = mention_count 大者存活，同 count 取名称长者（拼写变体通常更短），再同取 rowid 小者（先创建）——count 相等时方向不定会误删正主，见测试注释；mergeEntities 内部有存在性守卫，候选对引用已合并实体时跳过）；**substring**（子串包含「数据库」⊂「向量数据库」）**可能是上下位而非同义，不自动合**——错误合并污染共现边与实体检索，代价大于漏合，只作「词典建议」：`GET /entities/duplicates` 返回 `{groups(兼容), typo_groups, suggest_groups}`，Web 实体页候选「加入词典」一键把少提及方写成标准名别名（GET /term-dict → 合并 terms → PUT → 自动 rebuild 归并，声明式可审计可回滚），逐对 merge 的 ConfirmDialog 流程已移除；`POST /entities/:id/merge` API 保留（MCP/工具调用方）。长度 4 的 ASCII 近拼写（rust/rush）降级为不提示（宁漏合不错合）

- **LaTeX/公式渲染（阅读态 v1）**：块级用 ```math 围栏（latex/katex/tex 别名，复用 code_block 零存储改动），行内 `$...$` 走 `web/src/lib/katex.ts` 的 `INLINE_MATH_SRC`（**Pandoc 规则**：开 $ 后非空格、闭 $ 前非空格、闭 $ 后不跟数字——防「$100 与 $200」「$5-$10」货币误伤；不匹配 `$$`，测试在 `lib/__tests__/inlineMath.test.ts`）。KaTeX 懒加载（首个公式才拉库+CSS，同 mermaid 模式），**版本必须钉在 `^0.16.45` 与 mermaid 依赖对齐**（bun isolated 安装下直接装 0.18 会打出两份 260KB chunk）；`trust:false` 默认禁用 \href/\html*，renderToString 输出可安全 innerHTML。BlockRenderer 的 INLINE_RE 改为数组 source 组合（math 组在 code 之后、bold 之前，后续加备选注意组号顺移）。未做：AI 聊天气泡（react-markdown 需 remark-math/rehype-katex）、CM 编辑器预览 widget、`$$` 语法
