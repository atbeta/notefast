...
| Bun 版本 | `.bun-version`（现钉 `1.3.14`） | CI / Docker 同源；升级须改此文件并验证 Windows engine 杀软误报（不走代码签名） |
...
- **资源库**：`GET /api/v1/assets` 列表 + Web `/resources`（本地/已外链、使用中/未引用）；缩略图可 lightbox 放大；未引用可删（`DELETE /assets/:id`，使用中拒绝）及一键清理未引用；图床配置仍在设置；**未配置图床时不露上传入口**（`useImageUploadEnabled`：`mode=auto` 且命令非空才显示资源页「上传」与阅读态「仅本地/重试」；否则点了必 400）；编辑器工具栏 ImagePlus →「上传本地」/「从资源库」（`AssetPickerDialog` 单选插入 `![](asset:<sha>)`）；不做相册/批量修图/DAM；`asset:` 对用户保持内部引用
...
- **快照剥离可重建/可再生的非核心数据**（`backup/snapshot.ts` 的 `stripSecondaryDataFromSnapshot`：清空 block_vectors/vec_blocks/vector_entries/generations + 置 stale + 清空 entity_changes 同步历史 + VACUUM，保留表结构）；备份与同步 compaction 快照均只含核心内容（KB~MB 级），恢复后语义搜索为空需手动 `POST /ai/index/rebuild` 重建
...
- `updated_at` 语义 = 内容最后编辑时间（未来 LWW 裁决字段，客户端 push 可自带），**不做增量同步游标**；拉取游标用 `entity_changes.seq`（AUTOINCREMENT 单调递增，blocks 表 trigger 驱动，`store/changeFeed.ts` 封装）；change feed 清理已落地：同步 compaction（`compactChanges` 覆盖远端快照并删旧 changes 段后）以快照锚点 seq 为唯一下界调 `pruneChanges` 裁本地历史行（锚点为 0 不裁——未配置同步/换存储位置后首次发布需从 0 全量回放，防静默漏数据），超窗客户端走全量快照重同步
- 文档分享：`shares` 表（doc_id → 公开 token，schema v4）独立存储，开关不写 blocks（不触发 updated_at/hooks/索引/change feed）；管理 API `GET/PUT/DELETE /docs/:id/share`（PUT 幂等，关闭即删记录，重开全新 token 旧链接永久失效；仅对未删除文档开放）；有效期 `expires_at` 默认 NULL=永不过期（Notion 同款），可选 1/7/30 天（以调整为起点重算），过期=未分享（读取时惰性清理，公开/管理端点统一只见未过期）；**删除文档级联清除 shares 行（docs/blocks/notebooks 三条删除路径），归档（PATCH status=archived）同样级联关闭分享，恢复文档/取消归档均不复活旧链接，需重新开启**；公开端点 `/share/:token`（markdown，`Cache-Control: no-store`）与 `/share/:token/assets/:sha256`（限本文引用的图片，非全站代理；`private, immutable` 缓存——浏览器可永久缓存但共享缓存/CDN 不得留存，防分享关闭后旧 URL 从缓存外泄；引用 sha 集合按 entity_changes `MAX(seq)` 做内存缓存，不写 Content-Length）挂在 `/api/*` 之外、无鉴权；Web 公开页 `/s/:token` 绕开 Layout/AuthPrompt，允许分享 inbox/archived 文档（显式行为覆盖默认过滤），ai_exclude 文档首次开启需 body 带 `confirm_ai_exclude: true`（否则 409 `ai_exclude_share_needs_confirm`，已开启后调有效期不再要求）；Web 分享面板为顶栏轻量 popover（非 modal），已分享顶栏用 Globe 态；`GET /docs/list` 响应带 `shared` 标记（仅有效分享，过期惰性清理），Web 文档列表项显示 Globe「已分享」徽标（与「AI 隐藏」徽标并列，store 层 `listSharedDocIds` 批量取）；MCP 分享工具（`mcp/tools/share.ts`）：`notefast_share_doc`（幂等开启/调有效期）/ `notefast_get_share` / `notefast_unshare_doc`，语义对齐 REST 且审计事件 source=mcp，**ai_exclude 文档一律 forbidden 无 confirm 通道**（confirm 流程仅留给 Web UI 人类用户，AI agent 不应公开「对 AI 隐藏」的文档），不触发 scheduleSyncNow（shares 不进 change feed/多端同步）
- 内部 AI 功能边界：只做「采集/理解/检索/维护」四类笔记核心行为；内置 chat 系统提示须明确 NoteFast 产品身份（`ai/prompt.ts`，非无身份通用聊天）；通用聊天客户端能力（多会话、语音对话、角色市场）与平台生态能力（Agent 运行时、插件市场）外放给 MCP 消费方；外部连接器未来可能做，已预留 `properties.source`（`{provider, external_id, synced_at}`）+ `findDocIdBySource()` 供 upsert
...
- **Web 原生壳适配**：壳层（内嵌 WKWebView / Tauri）以 `?native=1`（或 `macos|tauri|windows|linux`）加载入口 URL → `index.html` 内联防闪烁脚本给 `<html>` 加 `native-shell` class + `data-shell` 属性；CSS 下 `[data-drag-region]` 启用 `-webkit-app-region: drag`、可交互子元素（button/a/input/textarea/select/[contenteditable]）自动 `no-drag`（Layout 顶栏 / PageHeader 等已标注）；`API_BASE` 为相对路径 `/api/v1`，同源内嵌天然可用；免鉴权模式（未配置 AUTH_PASSWORD）下 `auth/mode` 返回 passwordRequired=false，Web 不弹登录（App.tsx 的 showAuthPrompt）。**品牌与壳边界**：侧栏不再放 Logo/店名/Beta（导航与身份解耦）；Tauri `TitleBar` 左侧恢复 `NoteFast`；macOS 靠系统标题栏/Dock 身份；浏览器靠标签页（移动顶栏抽屉仍可显示店名）。**macOS 壳为隐藏式标题栏**（`NoteFastApp.swift` 的 `.windowStyle(.hiddenTitleBar)`，无原生工具栏——新建/设置/同步状态全部由 web UI 呈现，原生侧只留菜单命令）：红绿灯悬浮叠在侧栏首行左侧，与 web 同高同左右边距（`--shell-top-inset: 0`）；折叠轨过窄时仅用 `--shell-traffic-top-collapsed: 28px` 下移展开钮；窗口拖拽走系统标题栏 + web `[data-drag-region]`；双击缩放：系统 titlebar 区由 `AppModel.attachWindowChrome` 本地监听，web 拖拽区由 `lib/nativeWindow.ts`（`windowZoom` 消息 / Tauri maximize）
- **原生客户端工程布局（clients/）**：按平台族分目录（非 Bun workspace，`bun --filter '*'` 不扫）——`clients/apple/` = Apple 全平台 Swift 包（macOS 现役 + iOS 未来同包多 target），`clients/tauri/` = Tauri 壳（当前仅 Windows 构建，Linux 后续同一工程）。**`clients/apple/` 为 SPM 包**：`Sources/NoteFast` 共享库（NoteFastClient / Models / EngineProcess）+ `Sources/NoteFastApp` macOS 可执行 target（SwiftUI + WKWebView），`EngineProcess` 用 `#if os(macOS)` 隔离（iOS 禁派生进程）；platforms 已含 `.iOS(.v17)`，共享库对 iOS simulator/device triple 均实测可编（iOS app target 未建）。`scripts/assemble-app.sh`（`clients/apple/scripts/`）组装 `dist/NoteFast.app`：`bun run build:engine` → `swift build -c release` → bundle（engine 产物注入 `Contents/Resources/engine/`）→ ad-hoc 签名（**engine 带 Bun JIT entitlements**：allow-jit 等，见 `Resources/notefast-server.entitlements`，意味着不能上 Mac App Store，走 Developer ID）→ `codesign --verify`。dev 模式（`swift run`）用 `NOTEFAST_ENGINE_DIR` 指向 `packages/server/dist-engine`；App 数据目录 `~/Library/Application Support/NoteFast/`。**退出链路**：App 发 SIGTERM → engine bootstrap drain（`closeAllSseStreams` 后实测 ~0.2s）。**打开即导入**：Info.plist 注册 .md/.markdown（LSHandlerRank=Alternate 不抢默认打开方式），双击/拖 Dock → 导入收集箱并打开（单篇跳文档页，多篇跳 /inbox）。**SwiftUI 下 macOS 文件打开事件走 `.onOpenURL` 的 file:// 分支，AppDelegate 的 openFile(s) 不触发**（`AppModel.handle(url:)` 先判 `isFileURL`，openFile(s) 仅兜底）；engine 未就绪时经 `pendingImportFiles` 排队、启动后 drain。**macOS 壳 WebView/进程守护（2026-08 补齐）**：`DocWebView` 导航策略——同源（127.0.0.1/localhost）放行、target=_blank 同源当前页打开、外链（http/https/mailto）一律 `NSWorkspace` 交系统浏览器（WebView 内跳走无返回手段）、下载（`shouldPerformDownload`/不可展示 MIME）转 WKDownload 存 ~/Downloads 重名加序号；`WebNavigator` 直持 weak webView（⌘[ ⌘] 后退前进、⌘+/-/0 pageZoom、⌘P printOperation、⌘, 设置）；**engine stderr 落盘 `~/Library/Logs/NoteFast/engine.log`**（>4MB 截断，父进程 fd 在 run() 后即关闭），**运行期崩溃自愈**——`EngineProcess.onUnexpectedExit`（`stopping` 标记区分主动停机）→ AppModel 置 failed 态走 FailureView 重试；**菜单栏图标 + 关窗不退出（2026-08 已定，替代关窗即退出）**：`StatusBarController`（NSStatusItem，SF Symbol 模板图标，显示主窗口/新建/收集箱/退出）+ AppDelegate `shouldTerminateAfterLastWindowClosed=false` + `applicationShouldHandleReopen` 重建窗口（自行 `openWindow` 后必须 `return false`，否则 AppKit 再开一扇→双空白窗）；窗口重建三件套——WindowGroup(id: "main")、ContentView.onAppear 注入 `openWindowAction`（Environment 只能视图侧取）、**`AppModel.start()` 幂等守卫 `engine?.isRunning != true`**（窗口重建重跑 .task，无守卫会二次 spawn 报 alreadyRunning；**engine 仍在跑时须对新建 WebView 重新 `navigate` 入口 URL**，否则 about:blank——`WebNavigator` attach 时回放 `lastURL`）；导航类入口（newDoc/openSettings/openInbox/深链/导入）统一先 `showMainWindow()`。**壳层四项（2026-08 第二波）**：① 更新检查改为手动——**启动不再静默打 GitHub**；设置「关于」页（`/settings/about`：Logo + 版本 + 检查更新，有新版只给下载链接不自动装）与帮助菜单「检查更新 / 关于」共用 `UpdateChecker`（`isNewer` = `!SemVer.isAtLeast(current, min: latest)`）；web 侧 `lib/checkRelease.ts` 同源逻辑；非 Sparkle；② 版本不兼容改阻断页（ContentView 拦 MainView，`dismissedVersionWarning` 可「仍要继续」）；③ `notefast://search?q=` 深链走 **URL 参数** `/?palette_search=xxx`（非 CustomEvent——didFinish 时 React 监听未必就绪，URL 方案零时序问题），web Layout 消费后 `setPalettePrefill` + 开面板 + replace 清参防刷新复触发，CommandPalette 加 `initialQuery` prop；④ 系统通知桥：`WKScriptMessageHandler` 名 `notefast`（{type:'notify',title,body}）→ `NotificationManager`（**`usable = bundleURL 后缀 == "app"` 守卫——swift run 无 bundle 时 UNUserNotificationCenter 会崩**；前台也弹横幅靠独立 delegate）；web 侧 `lib/nativeNotify.ts`（`window.webkit?.messageHandlers` 浏览器/Tauri 天然 no-op），首个触发点 = useSyncStatus 失败转场（模块级 `lastNotifiedError` 文本去重，恢复清零后可再通知；双 hook 实例靠去重锚天然不双弹）。**已移除原生后退/前进条（原 NavStripView）**：⌘[ / ⌘] 与菜单仍可用；`WindowAccessor` 仅用于安装标题栏双击缩放。主题桥 `{type:'theme'}` 仍保留（消息通道已挂好），目前无原生控件需跟随。**Tauri 侧同能力**：`tauri.conf.json` 的 `bundle.fileAssociations` 注册 .md（NSIS 写「打开方式」注册表项，不设默认）；冷启动文件路径在 argv、`single_instance` 回调带第二实例 argv，两路都进 `src-tauri/src/import.rs`——等 engine 就绪（engine 由前端启动页拉起）→ 裸 TcpStream POST `/import/markdown`（status=inbox，回环 trustedLocal 免鉴权，不引 HTTP 依赖）→ 延时约 1.8s 再 `win.eval` 导航（盖过启动页最短停留+淡出，否则目标跳转被启动 replace 覆盖）；本机为 macOS 只能 cargo check，Windows 端到端需在 Windows 机验证。**Windows Tauri 冷启动**：勿在 `tauri.conf.json` 写死深色 `backgroundColor`（亮色系统会先黑再白）；`visible: false` → setup 按系统主题设底色（亮 `#fafafa` / 暗深色）再 `show()`；启动页 CSS 默认亮色、暗色走 `prefers-color-scheme`；启动页最短停留 + 淡出后再 `location.replace`（`MIN_SPLASH_MS`），跳转前窗口底色对齐主 UI（亮 `#fff` / 暗 `#191919`）。**Windows portable Release**（`windows-release.yml`）：zip 必须含 `notefast.exe`+`engine/`（对齐 NSIS `$INSTDIR`，缺 engine 不可用）；`Attach portable zip` 须真 bash（勿在 `shell: bash` 里塞 PowerShell）；体积 `<20MB` 直接失败防漏打 engine；artifact 上传放在 release attach 之前；**engine 可执行文件易触发企业杀软（如 Downloader.Win.Bitser）误报**——不走付费代码签名缓解（证书贵；Azure 个人签仅美加），靠钉 Bun（`.bun-version`）与发版前本机/CI 验证
- Web 阅读器样式与交互，替代原裸 textarea：底层始终是 Markdown 源码（零转换、无 round-trip 风险），装饰层做混合渲染（Typora-lite）。**排版体系与阅读态一致**（对齐 Obsidian 做法：编辑态复用阅读排版）：正文 16px `var(--font-sans)` / 行高 1.75，标题 = BlockRenderer 的 28/24/20px（1.75/1.5/1.25em），`var(--font-mono)` 只用于行内代码与代码块（0.85em + 淡底色），标记符压暗；图片 / 表格 / 块级公式 widget 预览。**高亮样式两个坑**：① `editorHighlight` 必须定义泛型 `t.heading` 基准规则——`defaultHighlightStyle`（fallback）给泛型 heading 加了 bold+underline，而 lezer 给「表格表头行」打的正是泛型 heading（`TableHeader/...: tags.heading`），不定基准就会被 fallback 命中；② **lezer 会把「ATX 标题后无空行直接跟的表格」误解析为 SetextHeading2**（表头行+分隔行=文字+下划线），该表格在语法树里不是 Table——源码态表头会套用 heading2 样式（仅观感问题，阅读态 core 解析与 tablePreview 的正则扫描都不受影响）；调试编辑器语法树可用控制台 `__cmView`（CodeMirrorEditor 挂载时暴露在 window 上）。要点：**block 级 Decoration 必须由 StateField 提供**（ViewPlugin 只支持行内装饰）；widget 统一模式为「光标/选区在块内显示源码、块外渲染 widget，点击 widget 光标入块（mousedown 必须 `preventDefault + stopPropagation`，否则 CM 指针选区在 widget 销毁后把光标映射到块外，立即弹回 widget）」，实现见 `cm/imagePreview.ts`、`cm/tablePreview.ts` 与 `cm/mathPreview.ts`（表格块检测对齐 core/markdown.ts 的 isTableRow/isTableDelimiter，跳过 ``` 围栏内部防误判）；**CM 只渲染视口行**（.cm-line 仅视口内几十行，DOM 探测长文档须先滚动）；**改 cm/ 主题后 HMR 不会重建 EditorView，必须整页刷新才生效**（EditorView 在 useEffect 里只创建一次）；列表/引用续行与空项退出用 lang-markdown 自带 `insertNewlineContinueMarkdown`；` ``` ` + Enter 展开空代码块是自定义 keymap（`cm/keymap.ts`）；AI ghost text 走 `cm/ghostText.ts` 的 StateField + widget（Tab 接受 / Esc·输入取消）；图片粘贴/拖拽经 `EditorView.domEventHandlers` 转交 `useImageUploader.uploadImage`；括号配对用 `closeBrackets()`。工具栏与 hooks 通过 `CodeMirrorEditorHandle` 命令式 API（insertAtCursor / wrapSelection / getSelectionText）与编辑器交互，签名与旧 textarea 版一致。旧 `useEditorKeyboard.ts` / `AiGhostOverlay.tsx` 已删除——原 BLOCK_TRIGGER 正则第二组是非捕获组 `(?:...)`，`m[2]` 恒为 undefined，Enter 会插入 "undefined " 文本，即用户反馈的「编辑乱码」来源，已随重写消除
- **实体词典（term-dict，实体校准层）**：`server/src/termDict.ts` + `data/term-dict.json`（`{version, terms: [{name, aliases[], kind?}]}`，默认无文件 = 空词典 = 检索行为零变化；sample 见 `docs/term-dict.sample.json`，行业内容（半导体/软件…）只是词典内容，软件不绑定行业）。匹配键统一 `normalizeEntityName`（trim/lowercase/去首尾标点/压缩空白），**精确匹配不子串**（「晶圆」是「晶圆厂」子串会误伤）。三处挂钩：① 抽取端 `registerMentions`（ai/entities.ts）优先 `resolveDictTerm` ——别名锚点直接收敛到标准名实体（display=标准名、kind 覆盖），声明层优先于手工合并别名字典；② 查询端 `lexicalSearch`（term → 组内 OR 展开 `[原词, 标准名, ...别名]`，**组间 AND/OR 语义不变宽**，整句 100 分按展开前原串判断，FTS MATCH 全组构建——CJK 组以引号短语进 AND 约束，防「wafer 光刻」只查 wafer 变体漏光刻约束）；③ 实体路 `entitySearch.matchEntities` 别名反向 resolve 到标准名参与精确/子串匹配。API `GET/PUT /api/v1/term-dict`（zod 结构校验 + saveTermDictToDisk 语义校验：标准名 ≥2 字/不重复/别名≠标准名，保存即自动归并）+ `POST /rebuild` 手动存量归并（幂等：先 upsert 标准实体 + kind 覆盖，再把别名/标准名命中的既有实体 `mergeEntities` 进标准实体——提及迁移、count 重算、旧名登记别名全复用现有路径）；**Web 设置页「实体词典」tab 是唯一写入入口**（结构化编辑器 + 导入/导出 + 保存 + 立即归并 + 清空）——实体页重复建议走直接合并实体，不写词典
- **CJK 问句前后缀剥离（查询端）**：`lexicalSearch` 的 `cjkCoreTerm` 前缀剥离（「什么是XXX」）+ **尾部问句剥离**（`CJK_QUERY_SUFFIX_RE`：「向量数据库怎么选」「红烧肉怎么做才好吃」「卡片笔记应该怎么写」→ 核心词，容忍「应该/要/想」中缀与「才/要/更」+ 短尾补语；中置式「怎么选向量数据库」不处理，留给语义路/词典）。**剥离词表冻结、只收疑问/指示词**（业界对照：大厂 query 理解用分词+停用词低权重而非删词，「配置/选择/了解」等名词词头高频动词已移出——「配置中心」剥成「中心」是纯损失；动词措辞差异（如何选择X vs 文档写怎么选X）由 term-dict 别名兜底，mock eval 移除三词后全量指标零变化）。**全半角**：core `fullToHalfWidth`（全角 ASCII 区 → 半角）用于查询 term 归一（`lexicalSearch` terms、`entitySearch` 候选名）；LIKE 组内每个 variant 再补 `halfToFullPunct` 标点/数字全角形态（**字母保持半角**——中文文档惯例是「RAG（检索增强生成）」混合写法），Cond/Params 共用 `groupForms` 展开保证占位符一一对应；词典匹配键 `dictKey = fullToHalfWidth(normalizeEntityName(x))`（**实体表归并键 normalizeEntityName 不含全半角转换**，存量稳定）。eval fixtures 已补 3 篇语料（RAG/检索增强、sqlite-vec 使用、向量数据库选型）+ 10 条查询（suffix_question/abbr_expansion/width_variant/alias_synonym 类型），mock 全量 70 条可跑；「向量库怎么用」类同义词查询与「sqlite_vec」连接符变体在无词典时词法 miss 属预期（term-dict/语义路兜底，评测注释已写明）
- **实体重复处理：信号分桶，自动合拼写变体 + 词典建议，不逐对人工合并**（`store/entities.ts` 的 `findPotentialDuplicates` + `api/entities.ts`）：`DuplicateGroup.signal` 分两类——**typo**（纯 ASCII 名、长度 ≥5、编辑距离 ≤2，qdrant/qdrnt）高置信同义 → 实体页加载时 `POST /api/v1/entities/duplicates/auto-merge` 自动合（有副作用故 POST 不挂 GET；合并方向 = mention_count 大者存活，同 count 取名称长者（拼写变体通常更短），再同取 rowid 小者（先创建）——count 相等时方向不定会误删正主，见测试注释；mergeEntities 内部有存在性守卫，候选对引用已合并实体时跳过）；**substring**（子串包含「数据库」⊂「向量数据库」）**可能是上下位而非同义，不自动合**——错误合并污染共现边与实体检索，代价大于漏合，实体页作为「可能同一实体」建议展示（`GET /entities/duplicates` 返回 `{groups(兼容), typo_groups, suggest_groups}`），**人工确认后直接合并**（逐组「合并」/「全部合并」带 ConfirmDialog 列方向预览；走 `POST /entities/:id/merge` → mergeEntities：提及迁移 + 旧名登记 entity_aliases；方向 = mention_count 大者存活）——**不写词典**（词典是设置页唯一写入入口的人工策展层；此前走「加入词典」批量写入 term-dict，声明层被低质量建议污染且用户无感知，已改）；合并后旧名的检索由 entitySearch 的 entity_aliases 反查兜住（与词典别名同级，详见该文件头注释）长度 4 的 ASCII 近拼写（rust/rush）降级为不提示（宁漏合不错合）

- **LaTeX/公式渲染（阅读态 v1）**：块级用 ```math 围栏（latex/katex/tex 别名，复用 code_block 零存储改动），行内 `$...$` 走 `web/src/lib/katex.ts` 的 `INLINE_MATH_SRC`（**Pandoc 规则**：开 $ 后非空格、闭 $ 前非空格、闭 $ 后不跟数字——防「$100 与 $200」「$5-$10」货币误伤；不匹配 `$$`，测试在 `lib/__tests__/inlineMath.test.ts`）。KaTeX 懒加载（首个公式才拉库+CSS，同 mermaid 模式），**版本必须钉在 `^0.16.45` 与 mermaid 依赖对齐**（bun isolated 安装下直接装 0.18 会打出两份 260KB chunk）；`trust:false` 默认禁用 \href/\html*，renderToString 输出可安全 innerHTML。BlockRenderer 的 INLINE_RE 改为数组 source 组合（math 组在 code 之后、bold 之前，后续加备选注意组号顺移）。**AI 聊天气泡公式（v1）**：ChatMarkdown 加 `remark-math@^6`（纯解析，不装 rehype-katex——KaTeX 仍走 lib/katex.ts 懒加载单例，主包不进 katex）；code 分派最前面先判 `lib/chatMath.ts` 的 `classifyChatMath`（math-inline → MathInline / math-display 与 ```math 别名围栏 → MathBlock，**必须先于 isBlock 判定**，否则 `language-math math-inline` 被误判块级）；mdast-util-math 输出约定与货币无守卫行为钉在 `lib/__tests__/chatMath.test.ts`（`$5 - $10` 会被解析为 inlineMath，已知接受项，靠 prompt 规则 12 + MathInline 回退兜底；单行 `$$x$$` 是 inline 非 display）；server `ai/prompt.ts` 中英规则 12 约定模型只输出 `$...$` 与 ```math 围栏（模板字符串内反引号须 `\`` 转义、`\[` 须 `\\[`）。**CM 编辑器块级公式预览已落地**（`cm/mathPreview.ts`，照 tablePreview 模式，见编辑器条目）。未做：阅读态 `$$` 语法、编辑器行内 `$...$` 预览（需解 code span 排除（syntaxTree 判断）与异步渲染缓存两个中等问题，v2 候选）
- **窗口 chrome 方向**：方案 1 已落地——**品牌跟系统铬走**（Tauri TitleBar 恢复店名；侧栏去品牌行；收起并进 ⌘K 行右侧；macOS 与 web 同高、红绿灯叠侧栏、折叠态才下移顶钮）；壳差异只许出现在 chrome 边界（`TitleBar` / `--shell-traffic-top-collapsed` / drag region / `nativeWindow`），禁止在 Sidebar 导航深处按壳分叉。**终态仍可向 macOS 收敛的一条带**（窗口控制浮在「侧栏顶+内容顶栏」拼成的带上，砍 h-9 整条 TitleBar，注意 AIChatPanel top-9 联动与内容顶栏右侧 ~130px 净空），留待下一波 chrome 打磨；参考形态=RelayCraft 单顶带（但其居中命令栏/纯图标栏不适合 NoteFast）。**AI 入口**：左侧导航不常驻；桌面常驻在内容顶栏右侧（`AiChatTrigger` / `PageHeader`），另有 ⌘J、⌘K、移动顶栏与情境「问 AI」。**侧栏 IA**：笔记（含新建/收集箱/归档/回收站）/ 资源 / 关系（实体·图谱，默认可折）；侧栏搜索条去掉 placeholder 只留图标+⌘K（右侧收起）；文档阅读右栏约 `w-72`（勿默认拉到 AI 面板 400px）；右栏 Tab（含「相关 / Related」）用 `grid-cols-5` 均分+`truncate`（英文 `backlinks`→`Links`），勿横滚；语义邻居走 `GET /api/v1/docs/:id/related`（`services/docRelated.ts`：hybridSearch + `contextDocId`，`skipSemantic`/`skipRerank` 轻量路径，打开文档预取）

- **回收站与 MCP 删除闭环**：`GET /docs/trash`（store `listDeletedDocRows`，注册在 `/docs/:id` 之前防路由吞掉）+ Web `/trash` 页（侧栏「回收站」入口，复用归档页形态，不做搜索/批量/彻底删除）。恢复 `POST /blocks/:id/restore`（及 MCP `notefast_restore_block`）对文档根补 `scheduleDocIndex` + `reanalyzeDoc`——**删除时向量被 afterDelete 清、mentions 被物理 purge，不补则恢复的文档在语义检索与图谱里是「哑」的**（无 provider 时安全 no-op）；分享旧链接按既定语义不复活。MCP `notefast_delete_doc`：与 `DELETE /docs/:id` 同一事务语义（refs/mentions 级联 + 软删除 + deleteSharesByDocIds）、ai_exclude forbidden、审计 `source:'mcp'`、`scheduleSyncNow`（tombstone 需同步传播）；工具描述引导「勿用于日常整理」。不做：物理 purge、delete_block、chat agent 删除工具

- **chat 检索结果必须带 ID**：`ai/prompt.ts` 的 `buildContextBlock` 渲染 citations 时 head 行带 `doc_id` / `block_id`（字段本来就在 Citation 里，曾只渲染标题+正文——系统提示让模型「ID 从检索结果获取」无从获取，`notefast_read_doc`/`notefast_update_block` 都会踩）；`notefast_list_docs` 执行层对 `status` 做白名单校验（schema enum 只是给模型的提示，漏传默认 note 排除收集箱、传非法值静默空列表曾让「整理收集箱」误报 inbox 为空），非法值显式报错让 agent 自我纠正
- **打开即导入去重（路径 upsert + hash 判同）**：`importMarkdownSchema` 接受可选 `source{provider, external_id}`（zod strip unknown key，不加字段传了也白传）；`POST /import/markdown` 带 source 时 `findDocIdBySource` 命中且 `source.content_hash`（sha256(markdown)，`DocSourceRef` 新增可选字段）一致 → 200 `{doc, deduplicated: true}` 零副作用返回既有文档；**内容变了 → 新建一篇进收集箱（不覆盖应用内编辑），source 身份移交新文档**（旧文档 `stripDocSource` 剥 properties.source 成为普通笔记，findDocIdBySource 恒锚定最新篇）；回收站中的旧导入不挡重新导入（is_deleted=0 过滤）。壳层：Tauri `import.rs` post_import 传 `file-open` + canonicalize 绝对路径，Apple `AppModel.swift` 传 `file.standardizedFileURL.path`（body 是 [String: Any]，嵌套字典可直接编）；Swift 侧改动已经本机 `swift build` + `swift test` 验证。provider 名 `file-open` 已写入数据，勿轻改。Web `/import/file`、`/import/zip`、MCP 导入不带 source，行为不变
- **编辑器选区气泡（桌面 v1）**：`components/editor/SelectionBubble.tsx`（挂 MarkdownEditor），动作两项——问 AI（复用 `dispatchAskAi`，截断 600 字）与改写（`useAiWriting.streamRefine` 流式原地替换；「续写」已有 Ctrl+J 入口刻意不进气泡）。CM 侧三件套：`cm/selectionReport.ts`（非空选区 200ms debounce + `hasFocus` + 非纯空白 → `coordsAtPos(to)` 报锚点，清空/失焦/卸载报 null；**不用 mouseup/keyup**，键盘 shift 选区会漏）、`cm/refineReplace.ts` + `components/editor/refineSession.ts`（**首个 token 替换原选区 `[from, to)`**——曾漏传 to 退化成纯插入、原文残留重复，端到端测试已钉住；之后逐 token 把 `[from, from+已累积)` 替换为累积全文，`userEvent:'input'` 让 history 合并成 1~2 个 undo 步；apply 内同步 onChange 不算外部编辑，token 间隙的外部 docChanged 即取消流保留已替换内容——ghost dismiss-on-input 同款）；handle 加 `replaceRange`。**两个必记坑**：① 气泡容器 `onMouseDown` 必须 `preventDefault`，否则点击瞬间选区塌陷拿不到文本；② 无 DOM 测试环境，可测逻辑抽纯模块（SelectionReporter/RefineSession/buildReplaceRangeUpdate）做状态机级单测。气泡复用手写 popover 模式（portal + fixed z-[80] + Esc/scroll 关闭 + 边界 clamp），`<sm` 不渲染（matchMedia gate，与 BlockSurface 同决策）；**未配置 chat 模型时整个气泡不出**（挂载时 `/ai/capabilities` 探测，chat!==true 即隐藏——两个动作无 chat 都必然失败，探测失败按未配置处理，与 AIChatPanel 同款）；改写与 ghost 续写互斥（启动改写会取消进行中的 ghost）。**`/ai/write` 已按 chat 同款剥离 think**（`ai/writeStream.ts`：content 内嵌 `<think>` 块经 `ThinkStreamParser` 剔除、reasoning 独立字段天然不下发、 think 与正文间的分隔空白缓冲到首个非空白字符再下发——否则推理模型的思考过程会被当改写/续写结果写回编辑器；测试 `__tests__/writeStream.test.ts`）
- **POST /import/markdown（外部接入点，契约只加不改）**：默认 notebook_id 可选（缺省 `resolveNotebookId` 取第一个笔记本）、tags 入库前 normalize（`services/docImport.ts` 的 `normalizeDocTags`，docs.ts 同用）；source provider 命名 `web-clipper` / `ios-shortcut`，`file-open` 已占用勿动，external_id 网页类用规范化 URL（去 hash/utm_*）。**CORS**：`CORS_ORIGINS` 任一项为字面 `*` 时给 hono cors 传字符串 `'*'`（数组语义下 `'*'` 不匹配任何 origin，这是修复点），`allowMethods` 已补 PUT；**免鉴权 + `*` = 任意网页可读写全库**（README/docs 均有警示，仅限本地/内网；该组合启动时 index.ts 另有醒目告警）。**api_tokens scopes 已强制**：`middleware/auth.ts` 方法级拦截——非 GET/HEAD/OPTIONS 且 token scopes 不含 write → 403（env API_TOKEN / api.key / web-session / trustedLocal 直通不受影响；**MCP `POST /mcp` 全 POST，只读 token 同样被挡**）。使用文档 `docs/capture.md`（API 契约，仅供外部客户端开发者参考；Web UI 已不再提供引导）。**明确不做服务端 URL 抓取**（readability+DOM+SSRF 防护成本，客户端带正文已覆盖且能拿登录态内容；URL 已在 source.external_id 留位）
- **Markdown 格式保真（marker + 块间空行）**：parse 记 `properties.marker`（`+`/`*`/`-` 原始字符，regex 捕获组顺移注意），export 优先回写、存量无字段回退 `-`（零迁移）；**export 插入块间空行**（`blocksToMarkdown` traverse：连续 list_item 之间不空行防拆列表、quote 内部 compact 不空行防拆引用、Document 标题后空行；末尾 `\n{3,}`→`\n\n` 收敛保留）——此前段间无空行，外部 CommonMark 渲染会把多段并成一段（用户感知的「空行消失」）。**连续多个空行仍不保真**（parse 跳过空行是块模型结构决定，「Markdown 仅作表达」立场内；要保真需空 paragraph 块方案，连带 BlockRenderer 空块渲染与 export 改动，未做）
