...
- **窗口 chrome 方向**：Tauri TitleBar 已删应用名文字（左侧只留拖拽区+窗口控制，消除与侧栏品牌行重复）；**终态=向 macOS 收敛的一条带**（窗口控制浮在「侧栏品牌行+内容顶栏」拼成的带上，砍 h-9 整条 TitleBar，注意 AIChatPanel top-9 联动与内容顶栏右侧 ~130px 净空），待 chrome 打磨窗口期实施；参考形态=RelayCraft 单顶带（但其居中命令栏/纯图标栏不适合 NoteFast）。**AI 入口**：左侧导航不常驻；桌面常驻在内容顶栏右侧（`AiChatTrigger` / `PageHeader`），另有 ⌘J、⌘K、移动顶栏与情境「问 AI」。**侧栏 IA**：笔记（含新建/收集箱/归档/回收站）/ 资源 / 关系（实体·图谱，默认可折）；侧栏搜索条去掉 placeholder 只留图标+⌘K；文档阅读右栏约 `w-72`（勿默认拉到 AI 面板 400px）；右栏 Tab（含「相关 / Related」）用 `grid-cols-5` 均分+`truncate`（英文 `backlinks`→`Links`），勿横滚；语义邻居走 `GET /api/v1/docs/:id/related`（`services/docRelated.ts`：hybridSearch + `contextDocId`，`skipSemantic`/`skipRerank` 轻量路径，打开文档预取）
