 ...
 *
 * 成功样本：与 expected.json 一致，且 parse → serialize → parse 语义不变。
 * 失败样本：钉住手写 parser「会丢 / 截断 / 认不出围栏」的现状；mdast 只允许这些条目改善。
 */

import { existsSync } from 'node:fs'
import { describe, expect, test } from 'bun:test'
import { parseMarkdownToBlocksLegacy, blocksToMarkdown } from '../../markdown'
...
describe('markdown corpus：成功样本（手写 parser 冻结）', () => {
  for (const id of SUCCESS_FIXTURES) {
    test(id, () => {
      const markdown = readFixtureFile('success', `${id}.md`)
      const meta = readMeta('success', id)
      const parsed = parseMarkdownToBlocksLegacy(markdown, 'corpus')
      ...
      const again = parseMarkdownToBlocksLegacy(exported, 'corpus')
