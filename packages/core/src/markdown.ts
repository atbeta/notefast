...
export function parseMarkdownToBlocks(markdown: string, notebookId: string): CreateBlockInput[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // 引用合并段：连续无空行的 `>` 行合成一块（引用是多行块，合段无损）。
    // 注意：段落（Paragraph）不做软换行合段——NoteFast 是块级编辑器，
    // 一行一块是原子语义，用户在编辑器里按回车分行的两行必须保留为两块，
    // 不能按 CommonMark「无空行=同一段」合并（否则输入两行保存后变一行）。
    if (
      last &&
      !blankSeen &&
      last.type === parsed.type &&
      parsed.type === BlockType.Quote
    ) {
