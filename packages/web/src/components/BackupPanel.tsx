    ...
    >
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="text-base font-medium text-foreground">{t('backup.enableBackup')}</div>
            ...
            {status.lastSuccessAt && (
              <div className="text-success flex items-center gap-1.5">
                ...
                {status.lastResult?.objectKey && (
                  <span className="font-mono ml-1 truncate max-w-[240px] text-xs opacity-80">{status.lastResult.objectKey}</span>
                )}
              </div>
            )}
            {status.lastError && (
          ...
          <div className="space-y-3 pt-4 border-t border-border/40">
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
              {points.map((p) => (
                <div
                  key={p.objectKey}
                  className="flex items-center justify-between gap-3 px-3.5 py-2.5 rounded-lg border border-border/60 bg-accent/10 text-sm hover:border-border transition-colors"
                >
            ...
            <p className="text-xs text-muted-foreground">
              {t('backup.restoreHint1')} <code className="font-mono bg-accent/50 px-1 py-0.5 rounded-md text-2xs">--dry-run</code> {t('backup.restoreHint2')}
            </p>
          </div>
        )}
      </div>
