  ...
  return (
    <div className={`my-5 rounded-lg border border-border bg-muted/30 overflow-hidden ${className}`.trim()}>
      <div className="flex items-center justify-between px-3 py-1.5 bg-muted/60 border-b border-border">
        <span className="text-xs font-mono text-muted-foreground/80">{label}</span>
      ...
      {loading && (
        <div className="px-4 py-8 text-center text-base text-muted-foreground">{t('mermaid.loading')}</div>
      )}

      {!loading && error && (
        <div className="p-4 space-y-3">
          <p className="text-base text-destructive">{t('mermaid.renderFailedWith', { error })}</p>
