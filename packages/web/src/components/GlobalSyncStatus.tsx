 ...
 *
 * 位置：文档页右侧有 w-56 的 rail（大纲/实体/历史 Tab），fixed right 会遮挡，
 * 因此 lg 屏让出 rail 宽（14rem + 0.75rem 边距），锚到主栏右上、rail 左侧。
 */
export default function GlobalSyncStatus() {
  ...
  // 文档页 rail 让位：lg 屏把胶囊移到主栏右上（rail 左侧），避免遮挡右侧 Tab
  ...
  return (
    <Tooltip label={tooltip}>
      <button
        ...
        className={`fixed top-3 right-3 z-40 flex items-center gap-1.5 h-7 pl-2 pr-2.5 rounded-full border border-border/60 bg-card/90 backdrop-blur text-[11.5px] text-muted-foreground shadow-sm hover:border-border hover:text-foreground transition-colors ${positionClass}`}
