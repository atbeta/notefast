...
export default function AutoLinkPanel({ docId, currentBlockId, onClose }: AutoLinkPanelProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-card border border-border rounded-xl overflow-hidden">
      <button
        ...
        className="flex items-center justify-between gap-2 px-4 py-3 bg-background/50 hover:bg-accent transition-colors shrink-0"
      >
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            disabled={triggering}
            className="text-[10px] text-muted-foreground hover:text-foreground px-2 py-1 rounded hover:bg-background disabled:opacity-50"
            title="重新分析此文档所有 block"
          >
            {triggering ? <Loader2 className="w-3 h-3 animate-spin" /> : '刷新'}
          </button>
          <button
            type="button"
            onClick={(e) => {
    ...
    <div className="rounded-lg border border-border/60 bg-background/40 p-3 space-y-2">
      ...
      <div className="flex items-center gap-1.5 pt-1">
        <button
          ...
          className="inline-flex items-center gap-1 px-2.5 py-1 rounded text-xs font-medium bg-primary text-primary-foreground hover:opacity-90 disabled:opacity-50 transition-opacity"
        >
