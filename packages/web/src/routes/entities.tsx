        ...
        {loading ? (
          ...
          <EmptyState
            ...
            action={ai.ready && !ai.chat ? (
              <Link
                to="/settings/ai"
                className="inline-flex items-center gap-1 rounded-md bg-primary px-3 py-1.5 text-sm font-medium text-primary-foreground hover:bg-primary-hover transition-colors"
              >
          ...
          <>
              ...
              {KIND_FILTERS.map((f) => {
                ...
                return (
                  ...
                  >
                    ...
                    {f.label}
                    <span className={`tabular-nums text-2xs ${active ? 'text-primary/70' : 'text-muted-foreground/60'}`}>
                      {count}
                    </span>
                  </button>
                )
            ...
            {autoMerged > 0 && (
              <div className="rounded-xl border border-success/20 bg-success-soft px-3.5 py-2.5 flex items-center gap-2 text-sm text-foreground">
              ...
              <div className="rounded-xl border border-warn/25 bg-warn/5 px-3.5 py-3">
                <div className="flex items-center justify-between gap-2 mb-2">
                  ...
                  <button
                    ...
                    className="shrink-0 inline-flex items-center gap-1 rounded-md border border-border bg-background hover:bg-accent hover:border-foreground/20 px-2 py-1 text-xs text-foreground transition-colors disabled:opacity-50"
                  >
                    {adopting ? (
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="ml-auto shrink-0 inline-flex items-center gap-1 rounded-md border border-border bg-background hover:bg-accent hover:border-foreground/20 px-2 py-1 text-xs text-foreground transition-colors disabled:opacity-50"
                        >
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            <h3 className="font-medium text-md text-foreground tracking-[-0.005em] truncate">
                              {e.display}
                            </h3>
                          ...
                          {e.description && (
                            <p className="text-sm text-muted-foreground/80 mt-1 leading-relaxed flex items-start gap-1.5">
                              {e.description_source === 'dict' && (
                                <span className="shrink-0 inline-flex items-center gap-1 rounded-md border border-primary/20 bg-primary-soft px-1.5 py-px text-2xs text-primary font-medium">
                                  {t('graph.dictSource')}
                                </span>
                              )}
                          ...
                          <div className="flex items-center gap-2">
                            {e.description_source !== 'dict' && (
                              <button
                                ...
                                className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md border border-border/70 bg-background text-xs text-muted-foreground hover:text-foreground hover:border-foreground/30 transition-colors disabled:opacity-50"
                              >
                                {generatingId === e.id ? (
                                  <Loader2 className="w-3 h-3 animate-spin" strokeWidth={1.75} />
                                ) : (
                            ...
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md border border-border/70 bg-background text-xs text-muted-foreground hover:text-foreground hover:border-foreground/30 transition-colors"
                            >
