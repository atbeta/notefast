...
export default function InboxPage() {
  ...
  const refresh = useCallback(async () => {
    setLoading(true)
    try {
      const r = await api.get<{ count: number; items: InboxItem[] }>(`/auto-link/inbox?status=${filter}&limit=200`)
      setItems(r.items)
    } catch {
      setItems([])
    } finally {
...
function StatusBadge({ action }: { action: InboxItem['action_status'] }) {
  const map: Record<InboxItem['action_status'], { label: string; cls: string }> = {
