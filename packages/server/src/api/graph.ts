/**
 * 图谱 API —— 实体共现图（图谱 UI 的数据源）
 *
 * - GET /api/v1/graph?center=&center_type=&depth=&min_mention=&max_nodes=&max_edges=&kind=
 *
 * center 为实体或文档 id（锚点，BFS 扩展邻居）；缺省 = 全库 top-N 总览。
 * min_mention 只约束邻居实体（锚点自身始终包含）；kind 逗号分隔多值。
...
import { GRAPH_LIMITS, queryGraph, type GraphCenter } from '../store/graph'
