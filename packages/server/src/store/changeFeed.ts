 ...
 *
 * 消费方是多端同步协议（sync/protocol.ts）：publish 按 seq 把增量导出为
 * changes/ 分段，compaction 覆盖全量快照后以快照锚点为下界调 pruneChanges
 * 裁剪本地历史行；超出快照窗口的端走全量快照重同步。
 */
 ...
 * 唯一下界 = 已被远端快照覆盖的 seq（compactChanges 成功后的快照锚点）：
