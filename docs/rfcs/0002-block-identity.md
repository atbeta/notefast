...
| 1000 文件首次对账 | < 30s | **8.6s**（117 files/s，18k 块，峰值 RSS 202MB） |
| 10k 文件首次对账 | < 5min | **269.1s**（37.2 files/s，180k 块，峰值 RSS 712MB）—— 达标但余量仅 10% |
| 文件变更 → SQLite 可见 | < stabilityMs + 200ms | **323ms**（1000 文件）/ **376ms**（10k）/ 382ms（迁移走查） |
