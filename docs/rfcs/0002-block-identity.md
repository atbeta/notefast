...
| 1000 文件首次对账 | < 30s | **8.6–9.4s**（107–117 files/s，18k 块，峰值 RSS 196–204MB） |
| 10k 文件首次对账 | < 5min | **258.6–279.8s**（35.7–38.7 files/s，180k 块，峰值 RSS 650–813MB）—— 达标但**余量 < 15%**（最差一次 6.7%） |
| 文件变更 → SQLite 可见 | < stabilityMs + 200ms | **315–333ms**（1000 文件）/ **366–369ms**（10k）/ 382ms（迁移走查） |
