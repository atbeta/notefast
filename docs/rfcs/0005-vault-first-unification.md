...
启动时（仅 vault 模式）判定 watcher 后端，顺序如下：

1. `VAULT_USE_POLLING` 显式给出 → 直接用它
2. vault 根所在文件系统是虚拟 / 网络盘（`fuse` / `virtiofs` / `9p` / `nfs` / `cifs` / `smb` …，读 `/proc/self/mounts`）→ 轮询
3. 否则在 vault 根写隐藏探测文件实测原生事件，无事件 → 轮询

**实际生效的模式**报进 `/vault/status`（`watcher_mode` + `polling_auto`）。

为什么必须有第 2 步（实测教训）：在 OrbStack + virtiofs 的 bind mount 上，容器内写文件会触发事件，但**宿主机**新建的文件迟迟不进索引——单文件探测只写自己这一侧，测不出这个方向，会给出假阳性「原生可用」。文件系统类型是容器里能拿到的、可靠的先验信号。

理由：Linux 宿主上的本地 bind mount 能投递 inotify，硬性统一成轮询等于白吃 1 秒延迟。这里要的是**行为一致**（自动得出正确模式），不是配置一致。

探测的能力边界（实测）：单文件事件在真实路径与符号链接路径（`/tmp` → `/private/tmp`）上都是 ~12ms 到达，所以「有没有事件」可判定；但 V-501 基准测到的 3776ms 是**批量变更**下 FSEvents 合并/重扫的延迟，单文件探测无法回答。这类落后由定时轻量对账兜底（V-404），不在本探测的职责内。
